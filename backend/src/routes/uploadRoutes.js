...
// S3 imports removed - keeping uploads local, only generated images go to S3
...
router.post('/interior', upload.single('image'), handleMulterError, async (req, res) => {
  try {
    ...
    // Save interior upload record to database with reference to storefront (keep local for AI processing)
    ...
    `, [
      ...
      req.file.filename,
      req.file.path, // Keep local path for AI processing
      req.file.size,
      req.file.mimetype,
      storefrontDesignId
    ]);
    ...
    // Create thumbnail for faster loading (local)
    ...
    res.json({
      ...
      data: {
        ...
        filePath: `/uploads/${req.file.filename}`, // Local path for frontend
...
router.post('/', upload.single('image'), handleMulterError, async (req, res) => {
  try {
    ...
    // Save upload record to database (keep local files for AI processing)
    ...
    `, [
      ...
      req.file.originalname,
      req.file.filename,
      req.file.path, // Keep local file path for AI processing
      req.file.size,
      req.file.mimetype
    ]);
